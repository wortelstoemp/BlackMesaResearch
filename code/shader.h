...
struct ShaderProgram
{
	...
	int CompileAndAttachShader(int programID, char** source, GLenum shaderType) const
	{
		if (*source == 0)
		{
			return 0;
	...
	char* fragmentSource)
{
	...
	CompileAndAttachShader(program->shaderProgram, &vertexSource, GL_VERTEX_SHADER);
	CompileAndAttachShader(program->shaderProgram, &tessControlSource, GL_TESS_CONTROL_SHADER);
	CompileAndAttachShader(program->shaderProgram, &tessEvalSource, GL_TESS_EVALUATION_SHADER);
	CompileAndAttachShader(program->shaderProgram, &geometrySource, GL_GEOMETRY_SHADER);
	CompileAndAttachShader(program->shaderProgram, &fragmentSource, GL_FRAGMENT_SHADER);
